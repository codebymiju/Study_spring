...
public class BoardController {
	...
	// 글쓰기 폼에서 글쓰기 버튼 클릭시 "write.bo" 서블릿 요청에 대한 매핑 > post
	...
	public String list (Model model) {
		// 데이터베이스 글목록 조회 비즈니스 로직 수행했다고 가정
		// 1. 전체 글 목록 저장하기 위한 List 객체 필요(boardList) ->  제네릭 (BoardVO)
		List<BoardVO> boardList = new ArrayList<BoardVO>();
		
		// 2. 1개 게시물 정보를 저장하는 boardBean 객체 생성 후 List 객체에 추가
		...
		// list() 파라미터에 Model 타입 선언 필요
		// 다음페이지에 뭔갈 가져가야 한다면 파라미터에 Model 객체 선언하기
